...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        lolcatjs.fromString(color(figlet.textSync('YUNO BOT', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        lolcatjs.fromString('[DEV] FARIS')
            ...
            if(mtcState === false){
                ...
                if(groups.length > groupLimit){
                    ...
                    if(chat.groupMetadata.participants.length < memberLimit){
                        ...
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Halo aku Yuno, Ketik help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'Yuno sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
