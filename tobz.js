...
INGAT JANGAN JUAL SCRIPT maslent KEPADA ORANG LAIN!
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '628165466368@c.us'
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Kamu belum cukup umur untuk menggunakan maslent, min 16 tahun\n\nKamu bisa mendaftar ulang dengan cara donasi terlebih dahulu, bales ${prefix}donasi\nHubungi Owner : wa.me/628165466368`, id) //if user is not registered
            }
        }
        ...
        const mess = {
            wait: ' [TUNGGU] Masi Di Proses Sabar Njer Orang Sabar di sayang Janda :b',
            magernulissatu: 'Harap Tunggu, BOT Lent Sedang Menulis Di Buku 1!',
            error: {
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            found === true 
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh maslent`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 12){
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh maslent!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh maslent!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
        ...
        function GenerateSerialNumber(mask){
            ...
            return serialNumber;
                ...
                if(body === '!mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin maslent!', id)
                ...
                if (body === '!unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner maslent!', id)
        ...
        case prefix+'restartlimit': // WORK IF YOU RUN USING PM2
            if(isOwner){
                ...
                oz.execCommand('node index').then(res=> {
                    ...
                    console.log("done restart limit", err);
                })
            }
            break
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner maslent!', id)
        ...
        case prefix+'ttp':
                if(isReg(obj)) return
                if(cekumur(cekage)) return
                if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', message.id)
                try
                {
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *!ttp2 [ Teks ]*, contoh *!ttp2 maslent*`, id)
        ...
        case prefix+'ttg':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, `hanya bisa di dalam grub Ngerti Ga cok? `, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            try {
                if (quotedMsgObj == null) {
        ...
        case prefix+'sgif': // MRHRTZ
            tobz.reply(from, `[WAIT] Bot Lent Sedang Memperoses....SABAR... orang sabar di sayang dajal`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *!blackpink [ Teks ]*, contoh *!blackpink maslent*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *!pornhub [ |Teks1|Teks2 ]*, contoh *!pornhub |Tobz|Dev maslent*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *!pornhub [ |Teks1|Teks2 ]*, contoh *!pornhub |Tobz|Dev maslent*`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *!glitch [ |Teks1|Teks2 ]*, contoh *!glitch |Tobz|Dev maslent*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *!glitch [ |Teks1|Teks2 ]*, contoh *!glitch |Tobz|Dev maslent*`, id)
            }
            break
        case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan maslent', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                        ...
                        await tobz.reply(from, `Format yang kamu masukkan salah, kirim ${prefix}daftar |nama|umur\n\ncontoh format: ${prefix}daftar |LENT|17\n\ncukup gunakan nama depan/panggilan saja`, id) //if user is not registered
                    }
                break
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin maslent', id)  
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `628165466368@c.us`)
        ...
        case prefix+'resetbadword':
            ...
            if(cekumur(cekage)) return
                    ...
                    if (!args.length === 1) return tobz.reply(from, 'Masukkan nomornya, *GUNAKAN AWALAN 62*\ncontoh: !resetbadword 628165466368 / !resetbadword @member') 
                    const nomer = args[1]
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin maslent!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
        ...
        case prefix+'bkp': // MFARELS
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', id) // MFARELS
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'nhentai': // SEARCH NHENTAI
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/628165466368', id)
          break
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'nulis':
                   ...
                   if(cekumur(cekage)) return
                   if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                   if (args.length == 0) return tobz.reply(from, `Membuat Text Sliding\nPemakaian: ${prefix}nulis2 [teks]\n\ncontoh: ${prefix}nulis2 Dimas`, id)
                    await tobz.reply(from, `sabar ya aq nuliss dulu....`, id)
                console.log('sedang menulis...')
                const nulis2q = body.slice(8)
                await tobz.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${nulis2q}&apikey=${vhtearkey}`, '', 'nih pemalas....', id)
        ...
        case prefix+'ptl':
            ...
            tobz.sendFileFromUrl(from, pep, 'pptl.jpg', 'Follow ig : maslent11', message.id)
            await limitAdd(serial)
            break
        case prefix+'neko':
            if(isReg(obj)) return
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin maslent!`, id)
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner maslent`, id)
                bctxt = body.slice(4)
                txtbc = `*「 maslent BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN maslent
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner maslent', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'hanya bisa di dalam grub Ngerti Ga cok? ', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin maslent', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin maslent', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin maslent', id)
            await tobz.sendText(from,'maslent DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'join':
            ...
            if (!isLink) return tobz.reply(from, 'Ini bukan link bodok...sopan kah begitu', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `hanya bisa di dalam grub Ngerti Ga cok? `, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan maslent`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan maslent`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan maslent atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'maslentadmin':
            let admn = `This is list of maslent Admin\nTotal : ${adminNumber.length}\n`
        ...
        case prefix+'gift': // Hanya Admin & Owner maslent yang bisa gift Limit
            if (!isAdmin, !isOwner) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin maslent!`, id)
                    const nomerr = arg.split(' ')[0]
                    const jmla = arg.split(' ')[1]
                    if(!nomerr) return tobz.reply(from, `Masukkan nomor yang akan di gift, ${prefix}gift [ @tagmember Jumlah ]\n=> Contoh : ${prefix}gift @628165466368 15`, id)
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner maslent!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin maslent!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner maslent!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin maslent!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner maslent!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin maslent!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin maslent!`, id)
        ...
        // LIST MENU
        case prefix+'runtime':
            tobz.reply(from, `maslent telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
            tobz.sendText(from, help(prefix, cts, pendaftar))
            break
        case prefix+'maslentgroup':
            tobz.reply(from, `Link Group maslent : https://chat.whatsapp.com/EIMG7qS9r0G8K45PfmLNxT\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin maslent', id)
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup maslent, Link Ada Di Bio atau Bisa Mengetik !maslentgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *!iklan*', id)
