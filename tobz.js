...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6287892670067@c.us'
        ...
        const tutor = 'https://i.ibb.co/4dWNwm9/Screenshot-2021-01-05-00-57-53-174-com-whatsapp.png'
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            found === true 
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Yuno`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 12){
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh Yuno!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Yuno!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
        ...
        function GenerateSerialNumber(mask){
            ...
            return serialNumber;
                ...
                if(body === '.mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Yuno!', id)
        ...
        case prefix+'restartlimit': // WORK IF YOU RUN USING PM2
            if(isOwner){
                tobz.sendText(from, 'RISET LIMIT SEDANG BERLANGSUNG')
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `6287892670067@c.us`)
            tobz.reply(from, 'Jika ingin bertanya tanya atau ingin donasi sikakan chat', id)
            break
        case prefix+'resetsticker':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Yuno!`, id)
            if (!args.length === 1) return tobz.reply(from, `Masukkan nomornya, *GUNAKAN AWALAN 62*\ncontoh: .resetsticker 628165466368 / .resetsticker @member`, id) 
            const nomebr = args[1]
        ...
        case prefix+'simi':
            ...
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Yuno!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
           ...
           try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    if (doujinveza.error) {
                        ...
                        tobz.sendFileFromUrl(from, shortenernh, `${title}.zip`, '_NHENTAI DOWNLOADER', id)
                        } catch (err){
                            console.log(err)
                        }
                    }    
            ...
            try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    if (getvide.error) {
                        ...
                        const captions = `*「 XVIDEO LENT DOWNLOADER 」*\n\n*Website* : XVideos\n\n*Ext* : MP4\n*Link* : ${shortvidxv2}\n*Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit*`
                        tobz.sendFileFromUrl(from, `https://sensorstechforum.com/wp-content/uploads/2019/07/xvideos-virus-image-sensorstechforum-com.jpg`, ``, captions, id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'setprefix':
            if(!isOwner) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner Yuno!`, id)
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Yuno!`, id)
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Yuno`, id)
                bctxt = body.slice(4)
                txtbc = `*「 YUNO BOT BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'omentionall':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Yuno', id)
        ...
        case prefix+'okickall':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Yuno', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Yuno', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Yuno', id)
            await tobz.sendText(from,'YUNO DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan Yuno`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan Yuno`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Yuno atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'yunoadmin':
            let admn = `This is list of Yuno Admin\nTotal : ${adminNumber.length}\n`
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Yuno!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Yuno!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Yuno!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Yuno!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Yuno!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Yuno!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Yuno!`, id)
        ...
        case prefix+'help':
            ...
            break
        case prefix+'yunogroup':
            tobz.reply(from, `Link Group Yuno : https://chat.whatsapp.com/BmbaLiLuwG7EWNMl2Eklww\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Yuno', id)
...
// By Gimenz
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Yuno, Link Ada Di Bio atau Bisa Mengetik .yunogroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *.iklan*', id)
